...
class UsersStartWithSeedsSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $seedItems = InventoryItem::where('inventory_item_category_id', $seedCategory->id)->get();

        if ($seedItems->count() === 0) {
            dd("ERROR: No existen items de semillas. Revisa InventoryItemSeeder.");
        }

        foreach (User::all() as $user) {

            $seedItem = $seedItems->first();
